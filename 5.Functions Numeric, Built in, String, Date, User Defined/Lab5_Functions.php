...
echo "<h2>3. Write a function to reverse a string.</h2>";
    function revString (string $rev):  string {//Time-Complexity: O(N) Linear Space-Complexity O(1)
        $chars = str_split($rev);
        $ans = "";
...
echo "<h2>4. Write a function to sort an array.</h2>";
    function sortArray(array $array):array {//Bubble sort time complixity: O(n²)
        $ans = array();
        $length = count($array);
...
echo "<h2>6. Write a PHP function that checks whether a passed string is a palindrome or not?</h2>";
    function isPalindrome(string $str): bool{//Time-Complixity = revString() O(N)
        return strtolower($str) == revString(strtolower($str));
    }


    try {
        echo isPalindrome("Mom")? "Yes Palindrome": "Not Palindrome";
    }
    catch(TypeError $e){
        echo '<b style="color:red">TypeError:</b> must be a string!';
    }
